...
$review = $_POST['review'];
    ...
    if (!in_array($extension, ['pdf'])) {
                echo '<script type = "text/javascript">
                     ...
                     ';
    } elseif ($_FILES['myfile']['size'] > 2000000) { // file shouldn't be larger than 2 Megabyte
        echo "File too large!";
    } else{
  ...
  $query=mysqli_query($conn,"SELECT * FROM `upload_files` WHERE `name` = '$filename'")or die(mysqli_error($conn));
        ...
        if (move_uploaded_file($file, $destination)) {
            $sql = "INSERT INTO upload_files (name, size, download, timers, admin_status, email, aname, issn, link, yearp, depart, ni, review) VALUES ('$filename', $size, 0, '$time', 'Admin', '$user', '$aname', '$issn', '$link', '$yearp','$depart','$ni','$review')";
